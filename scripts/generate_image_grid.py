...
def generate_image_grid(image_directory):
    ...
    for category in categories:
        ...
        moduleId = category.replace('_', '-').lower()
        output.append('Module <code>' + moduleId + '</code>')
        output.append(' ‎‎ | ‎‎ ')
        output.append('<a href="https://maxkeppeler.github.io/sheets-compose-dialogs/api/'+category+'/index.html" title="Module Documentation">API Documentation</a>\n')
        ...
        for light_image, dark_image in zip(light_images, dark_images):
            ...
            output.append("<tr>\n")
            output.append(f'<td width="25%"> <a style="text-align: center" href="https://github.com/maxkeppeler/sheets-compose-dialogs/blob/main/app/src/main/java/com/mk/sheets/compose/samples/{codeFile}.kt" title="Code">{codeFile}</a>\n </br>\n <img src="{lightImagePath / light_image.name}" /></td>\n')
            output.append(f'<td width="25%"> <a href="https://github.com/maxkeppeler/sheets-compose-dialogs/blob/main/app/src/main/java/com/mk/sheets/compose/samples/{codeFile}.kt" title="Code">{codeFile}</a>\n </br>\n<img src="{darkImagePath / dark_image.name}" /></td>\n')
