...
/**
 * Body component for the rating dialog.
 * @param config The configuration for the rating view.
 * @param body Implementation of the body.
 */
@ExperimentalMaterial3Api
@Composable
fun RatingBodyComponent(
    config: RatingConfig,
    body: RatingBody,
) {
    when (body) {
        is RatingBody.Custom -> body.body.invoke()
        is RatingBody.Default -> DefaultBodyComponent(config, body)
    }
}